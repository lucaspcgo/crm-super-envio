...
import {
  deleteDealDocumentAction,
  getDealDocumentUrlAction,
  uploadDealDocumentAction,
} from "@/lib/deals/documents";
import type { DealDocument } from "@/lib/deals/queries";

type Props = { orgSlug: string; dealId: string; initial: DealDocument[] };
...
export function DealDocuments({ orgSlug, dealId, initial }: Props) {
  ...
  function onFileChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    startUpload(async () => {
      const r = await uploadDealDocumentAction(formData);
      if (!r.ok) {
        toast.error(r.error);
        return;
      }
  ...
  async function handleDownload(path: string) {
    setBusyPath(path);
    const r = await getDealDocumentUrlAction({ orgSlug, path });
    setBusyPath(null);
    if (!r.ok || !r.data) {
  ...
  async function handleDelete(path: string) {
    ...
    setBusyPath(path);
    const r = await deleteDealDocumentAction({ orgSlug, path });
    setBusyPath(null);
    if (!r.ok) {
      toast.error(r.error);
      return;
